    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "Failed to map VC4 HVS registers into system address space. (status=0x%08lX, hvsMemoryResourcePtr->u.Memory.Start=0x%I64x, length=%d)",
            status,
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "Failed to map PixelValve registers into system address space. (status=0x%08lX, pvMemoryResourcePtr->u.Memory.Start=0x%I64x, length=%d)",
            status,
