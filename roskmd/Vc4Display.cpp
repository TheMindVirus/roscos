...
void VC4_DISPLAY::ResetDevice ()
{
    ...
    debug("Vc4DdiResetDevice() was called but is not implemented.");
}
    ...
    )
{
    ...
    if (rosKmdAllocation) {
        NT_ASSERT((((SIZE_T)(Args->PrimaryAddress.LowPart)) + (SIZE_T)(rosKmdAllocation->m_hwSizeBytes)) <= RosKmdGlobal::s_videoMemorySize);
    }
    ...
    debug("Successfully programmed VidPn source address. (PrimaryAddress.LowPart=0x%lx, physicAddress=0x%lx, virtualAddress = 0x%p)",
        Args->PrimaryAddress.LowPart,
        physicAddress,
    ...
    )
{
    ...
    // VSync interrupt
    if (intStat.VfpStart) {
        // debug("Notifying dxgkrnl of VSYNC interrupt.");
    ...
    currentVidPnSourceAddress()
{
    ...
    debug("Successfully constructed display subsystem. (this=0x%p, PhysicalDeviceObjectPtr=0x%p)",
        this, PhysicalDeviceObjectPtr);
}
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "Failed to map VC4 HVS registers into system address space. (status=0x%08lX, hvsMemoryResourcePtr->u.Memory.Start=0x%I64x, length=0x%016llX)",
            status,
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "Failed to map PixelValve registers into system address space. (status=0x%08lX, pvMemoryResourcePtr->u.Memory.Start=0x%I64x, length=0x%016llX)",
            status,
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "DxgkCbAcquirePostDisplayOwnership() failed. (status = 0x%08lX)",
            status);
        return status;
    }
    ...
    // Validate and store a pointer to the active display list
    {
        ...
        if ((dispList1.HEADE + ARRAYSIZE(displayListCopy.AsUlong)) >
            ...
            debug("Display list address is out of range");
            return STATUS_INVALID_DEVICE_STATE;
        }
        ...
        if (displayListCopy.PointerWord0 !=
            ...
            debug("The display list does not point to the BIOS buffer");
            return STATUS_INVALID_DEVICE_STATE;
        }
    ...
    *NumberOfChildrenPtr = CHILD_COUNT;     // represents the HDMI connector
    ...
    debug("Successfully started device.");
    return STATUS_SUCCESS;
}
...
void VC4_DISPLAY::StopDevice ()
{
    ...
    debug("Successfully stopped device.");
}
    ...
    )
{
    ...
    debug("Child relations reported successfully.");
    return STATUS_SUCCESS;
}
    ...
    )
{
    ...
    case StatusConnection:
        ...
        break;
    case StatusRotation:
        debug("Received StatusRotation query even though D3DKMDT_MOA_NONE was reported.");
        return STATUS_INVALID_PARAMETER;
    case StatusMiracastConnection:
        debug("Miracast is not supported.");
        return STATUS_NOT_SUPPORTED;
    ...
    )
{
    ...
    case DISPLAY_ADAPTER_HW_ID:
        ...
        break;
    default:
        debug("Invalid DeviceUid. (DeviceUid = %d)", DeviceUid);
        return STATUS_INVALID_PARAMETER;
    }

    debug("Successfully set power state. (DeviceUid = %d, DevicePowerState = %d, ActionType = %d)",
        DeviceUid, DevicePowerState, ActionType);
    ...
    )
{
    ...
    if (!SetPointerPositionPtr->Flags.Visible) {
        debug("Received request to set pointer visibility to OFF.");
        return STATUS_SUCCESS;
    } else {
        debug("SetPointerPosition should never be called to set the pointer to visible since VC4 doesn't support hardware cursors.");
        return STATUS_UNSUCCESSFUL;
    }
}
    ...
    )
{
    ...
    debug("SetPointerShape should never be called since VC4 doesn't support hardware cursors.");
    return STATUS_NOT_IMPLEMENTED;
}
    ...
    )
{
    ...
    if (IsSupportedVidPnPtr->hDesiredVidPn == 0) {
        ...
        debug("Returning IsVidPnSupported=TRUE for hDesiredVidPn=0.");
        return STATUS_SUCCESS;
    }
    ...
    NTSTATUS status = this->dxgkInterface.DxgkCbQueryVidPnInterface(
            ...
            &vidPnInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("DxgkCbQueryVidPnInterface() failed. (status = 0x%08lX, IsSupportedVidPnPtr->hDesiredVidPn = %p, this = %p)",
            status, IsSupportedVidPnPtr->hDesiredVidPn, this);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        debug("pfnGetTopology() failed. (status = 0x%08lX, IsSupportedVidPnPtr->hDesiredVidPn = %p, vidPnInterfacePtr = %p)",
            status, IsSupportedVidPnPtr->hDesiredVidPn, vidPnInterfacePtr);
        return status;
    }
    ...
    status = topologyInterfacePtr->pfnGetNumPaths(
            ...
            &numPaths);
    if (!NT_SUCCESS(status)) {
        debug("Failed to get number of paths in topology. (status = 0x%08lX, vidPnTopologyHandle = %p)",
            status, vidPnTopologyHandle);
        return status;
    }

    if (numPaths != 1) {
        debug("Returning 'not supported' for a topology that does not contain exactly 1 path. (numPaths = %Id, vidPnTopologyHandle = %p)",
            numPaths, vidPnTopologyHandle);
    ...
    if (status != STATUS_SUCCESS) {
        debug("Failed to get first path info from vidpn topology. (status = 0x%08lX, IsSupportedVidPnPtr->hDesiredVidPn = %p, vidPnTopologyHandle = %p)",
            status, IsSupportedVidPnPtr->hDesiredVidPn, vidPnTopologyHandle);
        return status;
    }
    ...
    {
        debug("Returning 'not supported' for out of range source or target id. (presentPathPtr->VidPnSourceId = %d, presentPathPtr->VidPnTargetId = %d)",
            presentPathPtr->VidPnSourceId, presentPathPtr->VidPnTargetId);
        NT_ASSERT(!IsSupportedVidPnPtr->IsVidPnSupported);
        return STATUS_SUCCESS;
    }

    debug("Returning 'supported' for vidpn. (IsSupportedVidPnPtr->hDesiredVidPn = %p)", IsSupportedVidPnPtr->hDesiredVidPn);
    ...
    )
{
    ...
    NTSTATUS status = this->dxgkInterface.DxgkCbQueryVidPnInterface(
            ...
            &vidPnInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("DxgkCbQueryVidPnInterface() failed. (status = 0x%08lX, EnumCofuncModalityPtr->hConstrainingVidPn, = %p, this = %p)",
            status, EnumCofuncModalityPtr->hConstrainingVidPn, this);
        return status;
    }
    ...
    status = vidPnInterfacePtr->pfnGetTopology(
        ...
        &topologyInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("pfnGetTopology() failed. (status = 0x%08lX, EnumCofuncModalityPtr->hConstrainingVidPn, = %p, vidPnInterfacePtr = %p)",
            status, EnumCofuncModalityPtr->hConstrainingVidPn, vidPnInterfacePtr);
        return status;
    }
    ...
    status = topologyInterfacePtr->pfnAcquireFirstPathInfo(
            ...
            &presentPathPtr);
    if (!NT_SUCCESS(status)) {
        debug("Failed to get first path info from vidpn topology. (status = 0x%08lX, EnumCofuncModalityPtr->hConstrainingVidPn, = %p, vidPnTopologyHandle = %p)",
            status, EnumCofuncModalityPtr->hConstrainingVidPn, vidPnTopologyHandle);
        return status;
    }
    ...
    while (status == STATUS_SUCCESS) {
              ...
              (EnumCofuncModalityPtr->EnumPivot.VidPnSourceId == presentPathPtr->VidPnSourceId)))
        {
            ...
            case STATUS_NOT_FOUND:
            {
                ...
                debug("This source does not have a pinned mode. Creating a new source mode set. (presentPathPtr->VidPnSourceId = %d)",
                    presentPathPtr->VidPnSourceId);
              ...
              (EnumCofuncModalityPtr->EnumPivot.VidPnTargetId == presentPathPtr->VidPnTargetId)))
        {
            ...
            case STATUS_NOT_FOUND:
            {
                ...
                debug("This target does not have a pinned mode. Creating a new target mode set. (presentPathPtr->VidPnTargetId = %d)",
                    presentPathPtr->VidPnTargetId);
        ...
        if (presentPathModified) {
            ...
            status = topologyInterfacePtr->pfnUpdatePathSupportInfo(
                    ...
                    &modifiedPresentPath);
            if (!NT_SUCCESS(status)) {
                debug("DXGK_VIDPNTOPOLOGY_INTERFACE::pfnUpdatePathSupportInfo() failed. (status = 0x%08lX)",
                    status);
                return status;
            }
        }
        ...
        status = topologyInterfacePtr->pfnAcquireNextPathInfo(
            ...
            &nextPresentPathPtr);
        if (!NT_SUCCESS(status)) {
            debug("pfnAcquireNextPathInfo() failed. (status = 0x%08lX, EnumCofuncModalityPtr->hConstrainingVidPn, = %p, vidPnTopologyHandle = %p)",
                status, EnumCofuncModalityPtr->hConstrainingVidPn, vidPnTopologyHandle);
            return status;
        }
    ...
    )
{
    ...
    debug("Received request to set visibility. (VidPnSourceId = %d, Visible = %d)",
        SetVidPnSourceVisibilityPtr->VidPnSourceId, SetVidPnSourceVisibilityPtr->Visible);
    ...
    )
{
    ...
    debug("DdiCommitVidPn() was called. (hFunctionalVidPn = %p, AffectedVidPnSourceId = %d, MonitorConnectivityChecks = %d, hPrimaryAllocation = %p, Flags.PathPowerTransition = %d, Flags.PathPoweredOff = %d)",
    ...
    NTSTATUS status = this->dxgkInterface.DxgkCbQueryVidPnInterface(
            ...
            &vidPnInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("DxgkCbQueryVidPnInterface() failed. (status = 0x%08lX, CommitVidPnPtr->hFunctionalVidPn, = %p, this = %p)",
            status, CommitVidPnPtr->hFunctionalVidPn, this);
        return status;
    }
    ...
    status = vidPnInterfacePtr->pfnGetTopology(
        ...
        &topologyInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("pfnGetTopology() failed. (status = 0x%08lX, CommitVidPnPtr->hFunctionalVidPn, = %p, vidPnInterfacePtr = %p)",
            status, CommitVidPnPtr->hFunctionalVidPn, vidPnInterfacePtr);
        return status;
    }
    ...
    status = topologyInterfacePtr->pfnGetNumPaths(
            ...
            &numPaths);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPNTOPOLOGY_INTERFACE::pfnGetNumPaths() failed. (status = 0x%08lX)",
            status);
        return status;
    }

    if (numPaths == 0) {
        debug("There are no paths in this topology.");
        return STATUS_SUCCESS;
    }
    ...
    status = vidPnInterfacePtr->pfnAcquireSourceModeSet(
            ...
            &smsInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPN_INTERFACE::pfnAcquireSourceModeSet() failed. (status = 0x%08lX)",
            status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPNSOURCEMODESET_INTERFACE::pfnAcquirePinnedModeInfo() failed. (status = 0x%08lX, sourceModeSetHandle = %p)",
            status,
            sourceModeSetHandle);
        return status;
    }
    ...
    {
        debug("VidPn source has different size than monitor. (pinnedSourceModeInfoPtr->Format.Graphics.PrimSurfSize = %d,%d, this->dxgkVideoSignalInfo.TotalSize = %d, %d)",
    ...
    status = topologyInterfacePtr->pfnGetNumPathsFromSource(
            ...
            &numPathsFromSource);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPNTOPOLOGY_INTERFACE::pfnGetNumPathsFromSource() failed. (status = 0x%08lX)",
            status);
        return status;
    }
    ...
    for (SIZE_T pathIndex = 0; pathIndex < numPathsFromSource; ++pathIndex) {
        ...
        status = topologyInterfacePtr->pfnEnumPathTargetsFromSource(
                ...
                &targetId);
        if (!NT_SUCCESS(status)) {
            debug("DXGK_VIDPNTOPOLOGY_INTERFACE::pfnEnumPathTargetsFromSource() failed. (status = 0x%08lX)",
                status);
            return status;
        }
        ...
        status = topologyInterfacePtr->pfnAcquirePathInfo(
                ...
                &presentPathPtr);
        if (!NT_SUCCESS(status)) {
            debug("DXGK_VIDPNTOPOLOGY_INTERFACE::pfnAcquirePathInfo() failed. (status = 0x%08lX, AffectedVidPnSourceId = %d, targetId = %d)",
                status,
                CommitVidPnPtr->AffectedVidPnSourceId,
                targetId);
            return status;
    ...
    debug("Successfully committed VidPn. (hFunctionalVidPn = %p, PrimSurfSize = %d, %d, VisibleRegionSize = %d,%d, Stride = %d, PixelFormat = %d, ColorBasis = %d, PixelValueAccessMode = %d)",
        CommitVidPnPtr->hFunctionalVidPn,
        pinnedSourceModeInfoPtr->Format.Graphics.PrimSurfSize.cx,
        pinnedSourceModeInfoPtr->Format.Graphics.PrimSurfSize.cy,
    ...
    )
{
    ...
    NTSTATUS status = tbl.pfnCreateNewModeInfo(
            ...
            &monitorModePtr);
    if (!NT_SUCCESS(status)) {
        debug("pfnCreateNewModeInfo() failed. (status = 0x%08lX)",
            status);
        return status;
    }
    ...
    *monitorModePtr = D3DKMDT_MONITOR_SOURCE_MODE();
    ...
    if (!NT_SUCCESS(status)) {
        if (status == STATUS_GRAPHICS_MODE_ALREADY_IN_MODESET) {
            ...
            debug("pfnAddMode failed. (status = 0x%08lX, RecommendMonitorModesPtr->hMonitorSourceModeSet = %p, monitorModePtr = %p)",
                status,
                RecommendMonitorModesPtr->hMonitorSourceModeSet,
                monitorModePtr);
            return status;
    ...
    )
{
    ...
    case DXGK_INTERRUPT_CRTC_VSYNC:
        if (EnableInterrupt) {
            debug("Enabling CRTC_VSYNC interrupt");
            ...
            debug("Disabling CRTC_VSYNC interrupt");
    ...
    )
{
    ...
    debug("DdiQueryVidPnHWCapability() was called. (hFunctionalVidPn = %p, SourceId = %d, TargetId = %d)",
    ...
    ) const
{
    ...
    NTSTATUS status = VidPnInterfacePtr->pfnCreateNewSourceModeSet(
            ...
            &smsInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPN_INTERFACE::pfnCreateNewSourceModeSet failed. (status = 0x%08lX)",
            status);
        return status;
    }
    NT_ASSERT(sourceModeSetHandle);
    ...
    // Create new mode info and add it to the source mode set
    {
        ...
        status = smsInterfacePtr->pfnCreateNewModeInfo(
                ...
                &sourceModeInfoPtr);
        if (!NT_SUCCESS(status)) {
            debug("DXGK_VIDPNSOURCEMODESET_INTERFACE::pfnCreateNewModeInfo() failed. (status = 0x%08lX)",
                status);
            return status;
        }
        ...
        *sourceModeInfoPtr = D3DKMDT_VIDPN_SOURCE_MODE();
        ...
        debug("Adding source mode. (PrimSurfSize = %d,%d, Stride = %d, PixelFormat = %d, ColorBasis = %d, PixelValueAccessMode = %d)",
        ...
        status = smsInterfacePtr->pfnAddMode(
                ...
                sourceModeInfoPtr);
        if (!NT_SUCCESS(status)) {
            debug("DXGK_VIDPNSOURCEMODESET_INTERFACE::pfnAddMode() failed. (status = 0x%08lX)",
                status);
            return status;
        }
    ...
    status = VidPnInterfacePtr->pfnAssignSourceModeSet(
            ...
            sourceModeSetHandle);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPN_INTERFACE::pfnAssignSourceModeSet() failed. (status = 0x%08lX)",
            status);
        return status;
    }
    releaseSms.DoNot();
    ...
    )
{
    ...
    NTSTATUS status = VidPnInterfacePtr->pfnAcquireTargetModeSet(
            ...
            &tmsInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPN_INTERFACE::pfnAcquireTargetModeSet() failed. (status = 0x%08lX)",
            status);
        return RosSanitizeNtstatus(status);
    }
    ...
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPNTARGETMODESET_INTERFACE::pfnAcquirePinnedModeInfo() failed. (status = 0x%08lX, targetModeSetHandle = %p)",
            status, targetModeSetHandle);
        return RosSanitizeNtstatus(status);
    }
    ...
    ) const
{
    ...
    NTSTATUS status = VidPnInterfacePtr->pfnCreateNewTargetModeSet(
            ...
            &tmsInterfacePtr);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPN_INTERFACE::pfnCreateNewTargetModeSet failed. (status = 0x%08lX)",
            status);
        return status;
    }
    NT_ASSERT(targetModeSetHandle);
    ...
    // Create new mode info and add it to the target mode set
    {
        ...
        status = tmsInterfacePtr->pfnCreateNewModeInfo(
                ...
                &targetModeInfoPtr);
        if (!NT_SUCCESS(status)) {
            debug("DXGK_VIDPNTARGETMODESET_INTERFACE::pfnCreateNewModeInfo() failed. (status = 0x%08lX)",
                status);
            return status;
        }
        ...
        status = tmsInterfacePtr->pfnAddMode(
                ...
                targetModeInfoPtr);
        if (!NT_SUCCESS(status)) {
            debug("DXGK_VIDPNTARGETMODESET_INTERFACE::pfnAddMode() failed. (status = 0x%08lX)",
                status);
            return status;
        }
        releaseModeInfo.DoNot();
    ...
    status = VidPnInterfacePtr->pfnAssignTargetModeSet(
            ...
            targetModeSetHandle);
    if (!NT_SUCCESS(status)) {
        debug("DXGK_VIDPN_INTERFACE::pfnAssignTargetModeSet() failed. (status = 0x%08lX)",
            status);
        return status;
    }
    releaseSms.DoNot();
    ...
    )
{
    ...
    if (SourceModePtr->Type != D3DKMDT_RMT_GRAPHICS) {
        debug("Pinned source mode is not a graphics mode. (SourceModePtr->Type = %d)",
            SourceModePtr->Type);
        return STATUS_GRAPHICS_INVALID_VIDEO_PRESENT_SOURCE_MODE;
    }
    ...
    {
        debug("Pinned source mode has a non-linear RGB color basis (ColorBasis = %d)",
            SourceModePtr->Format.Graphics.ColorBasis);
        return STATUS_GRAPHICS_INVALID_VIDEO_PRESENT_SOURCE_MODE;
    }

    if (SourceModePtr->Format.Graphics.PixelValueAccessMode != D3DKMDT_PVAM_DIRECT) {
        debug("Pinned source mode has a palettized access mode. (PixelValueAccessMode = %d)",
            SourceModePtr->Format.Graphics.PixelValueAccessMode);
        return STATUS_GRAPHICS_INVALID_VIDEO_PRESENT_SOURCE_MODE;
    }

    if (SourceModePtr->Format.Graphics.PixelFormat != D3DDDIFMT_A8R8G8B8) {
        debug("Pinned source mode has invalid pixel format. (SourceModePtr->Format.Graphics.PixelFormat = %d, D3DDDIFMT_A8R8G8B8 = %d)",
    ...
    )
{
    ...
    if (PathPtr->VidPnSourceId != 0) {
        debug("Path contains invalid source id. (VidPnSourceId = %d)",
            PathPtr->VidPnSourceId);
        return STATUS_GRAPHICS_INVALID_VIDEO_PRESENT_SOURCE;
    }

    if (PathPtr->VidPnTargetId != 0) {
        debug("Path contains invalid target id. (VidPnTargetId = %d)",
    ...
    {
        debug("Path contains a gamma ramp. (GammaRamp.Type = %d)",
    ...
    {
        debug("Path contains a non-identity scaling. (ContentTransformation.Scaling = %d)",
    ...
    {
        debug("Path contains a not-identity rotation (ContentTransformation.Rotation = %d)",
    ...
    {
        debug("Path has a non-linear RGB color basis. (VidPnTargetColorBasis = %d)",
