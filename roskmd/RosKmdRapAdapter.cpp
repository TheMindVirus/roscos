    ...
    OUT_PULONG              NumberOfChildren)
{
    ...
    {
        debug(
            "RosKmAdapter::Start(...) failed. (status=0x%08lX)",
            status);
        return status;
    }
    ...
    if (m_flags.m_isVC4)
    {
        ...
        {
            debug(
                "Failed to map memory for VC4 renderer registers. (status=0x%08lX, pResource=0x%p)",
                status,
                pResource);
            return status;
        }
        ...
        {
            debug(
                "Failed to open handle to rpiq device. (status=0x%08lX, rpiqDeviceName=%wZ)",
                status,
                &rpiqDeviceName);
            return status;
        }
        ...
        {
            debug(
                "Failed to power on VC4. (status=0x%08lX)",
                status);
            return status;
        }
    ...
    if (!RosKmdGlobal::IsRenderOnly())
    {
        ...
        {
            debug(
                "Failed to initialize display subsystem. (status=0x%08lX)",
                status);
            return status;
        }
    ...
    debug("RosKmdRapAdapter successfully started.");
...
NTSTATUS RosKmdRapAdapter::Stop ()
{
    ...
    debug("Stopping RosKmdRapAdapter");
        ...
        bool    bOn)
{
    ...
    {
        debug(
            "Failed to send IOCTL_MAILBOX_PROPERTY to rpiq. (status=0x%08lX, information=0x%016llX, sizeof(setPowerVC4)=0x%016llX, m_pRpiqDevice=0x%p)",
            status,
            information,
            sizeof(setPowerVC4),
            m_pRpiqDevice);
