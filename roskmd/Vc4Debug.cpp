    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "pfnAcquireFirstModeInfo() failed. (status = 0x%08lX, MonitorModesPtr->hMonitorSourceModeSet = %p)",
            status,
            MonitorModesPtr->hMonitorSourceModeSet);
        return;
    }
    ...
    while (status == STATUS_SUCCESS) {
        ...
        if (!NT_SUCCESS(status)) {
            debug(
                "pfnAcquireNextModeInfo() failed. (status = 0x%08lX, MonitorModesPtr->hMonitorSourceModeSet = %p)",
                status,
                MonitorModesPtr->hMonitorSourceModeSet);
            return;
        }
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "vidPnInterfacePtr->pfnAcquireSourceModeSet failed. (status = 0x%08lX)",
            status);
        return;
    }
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "smsInterfacePtr->pfnAcquireFirstModeInfo() failed. (status = 0x%08lX)",
            status);
        return;
    }
    ...
    while (status == STATUS_SUCCESS) {
        ...
        if (!NT_SUCCESS(status)) {
            debug(
                "smsInterfacePtr->pfnAcquireNextModeInfo() failed. (status = 0x%08lX)",
                status);
            return;
        }
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "VidPnInterfacePtr->pfnAcquireTargetModeSet() failed. (status = 0x%08lX)",
            status);
        return;
    }
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "tmsInterfacePtr->pfnAcquireFirstModeInfo() failed. (status = 0x%08lX)",
            status);
        return;
    }
    ...
    while (status == STATUS_SUCCESS) {
        ...
        if (!NT_SUCCESS(status)) {
            debug(
                "tmsInterfacePtr->pfnAcquireNextModeInfo() failed. (status = 0x%08lX)",
                status);
            return;
        }
...
void VC4_DEBUG::DumpVidPn (D3DKMDT_HVIDPN VidPnHandle)
{
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "DxgkCbQueryVidPnInterface() failed. (status = 0x%08lX, VidPnHandle = %p, this = %p)",
            status,
            VidPnHandle,
            this);
        return;
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "pfnGetTopology() failed. (status = 0x%08lX, VidPnHandle = %p, vidPnInterfacePtr = %p)",
            status,
            VidPnHandle,
            vidPnInterfacePtr);
        return;
    ...
    if (!NT_SUCCESS(status)) {
        debug(
            "Failed to get first path info from vidpn topology. (status = 0x%08lX, VidPnHandle = %p, vidPnTopologyHandle = %p)",
            status,
            VidPnHandle,
            vidPnTopologyHandle);
        return;
    ...
    while (status == STATUS_SUCCESS) {
        ...
        if (!NT_SUCCESS(status)) {
            debug(
                "pfnAcquireNextPathInfo() failed. (status = 0x%08lX, VidPnHandle = %p, vidPnTopologyHandle = %p)",
                status,
                VidPnHandle,
                vidPnTopologyHandle);
            return;
